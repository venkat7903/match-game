...
class MatchGame extends Component {
  ...
  decrementTime = () => {
    ...
    if (time <= 0) {
      ...
      this.setState({isMatchRunning: false})
    } else {
  ...
  getRandomNum = () => Math.floor(Math.random() * imagesList.length)

  getRandomImage = () => {
    const randomNum = this.getRandomNum()
    this.setState(prevState => ({
      score: prevState.score + 1,
      activeImage: imagesList[randomNum],
    }))
  }
  ...
  render() {
    const {isMatchRunning, score, time} = this.state
    return (
      <div className="match-game-app-container">
        <Navbar score={score} time={time} />
        <div className="sub-match-game-app-container">
          {isMatchRunning ? (
